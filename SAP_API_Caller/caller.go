...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-maintenance-item-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Item(maintenancePlan, maintenanceItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "MaintenanceItemItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": callObjectsData, "function": "MaintenanceItemCallObjects"})
	if err != nil {
		c.log.Error(err)
		return
	}
